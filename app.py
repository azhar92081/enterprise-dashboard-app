...
def load_data():
    ...
    for d in dates:
        for source in ["Organic Search", "Direct", "Social Media", "Paid Ads"]:
            ...
            traffic_data.append({
                "Date": d, "Source": source, "Visitors": visitors,
                "Bounce_Rate": np.random.uniform(30, 75),
                "Conversions": int(visitors * np.random.uniform(0.01, 0.08))
            })
...
with st.sidebar:
    ...
    selected = option_menu(
        ...
        default_index=0,
        styles={
            "nav-link-selected": {"background-color": "#1A365D", "color": "#00E5FF", "font-weight": "bold"},
...
# ==========================================
# PAGE 4: REAL AI DATA ANALYST (GEMINI)
# ==========================================
elif selected == "🤖 AI Analyst":
    st.title("🤖 Chat with Your Data (Gemini 2.5 Flash)")
